...
const PostCard = ({
  ...
  useEffect(() => {
    ...
    // Parse the song JSON string if it exists
    if (item?.song) {
      try {
        const songData = JSON.parse(item.song);
    ...
    <View style={[styles.container, hasShadow && shadowStyles]}>
      ...
      <View style={styles.content}>
        <View style={styles.postBody}>
          {item?.body && (
            <RenderHtml contentWidth={wp(100)} source={htmlBody} tagsStyles={tagsStyles} />
          )}
        </View>

        {/* Display the song information if it exists */}
